...
const T Text_ucase  = {  26, cset + (int)'A' };
const T Text_digits = {  10, cset + (int)'0' };
...
T Text_Cat(T s1, T s2)
{
  ...
  // s1 or s2 is null, so return the other
  if (s1.len == 0) {
    return s2;
  }
  if (s2.len == 0) {
  ...
  // The strings are already cat'd in memory, just extend s1
